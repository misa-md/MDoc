...
MISA-MD 模拟结束后，会产生二进制的原子信息文件，我们需要使用 md-tools 工具(v0.1.0)将其转化为可读的文本文件。
...
命令示例：
```bash
md-tools -f text -r 16 -i ./misa_md.origin.out -o origin.txt
```
...
```bash
md-tools -f xyz -r 16 -i ./misa_md_md.origin.out -o origin.xyz
```